/*
  This version of dlmalloc has been hacked by John Plevyak who
  releases all the hacks into the public domain as explained below.
*/
#define HAVE_MORECORE 1
#define MORECORE(S, M) (*(M)->morecore_pfn)(S, M)
...
#define DL_M_TRIM_THRESHOLD (-1)
#define DL_M_GRANULARITY (-2)
#define DL_M_MMAP_THRESHOLD (-3)
...
/*
  ...
  DL_M_TRIM_THRESHOLD     -1   2*1024*1024   any   (MAX_SIZE_T disables)
  DL_M_GRANULARITY        -2     page size   any power of 2 >= page size
  DL_M_MMAP_THRESHOLD     -3      256*1024   any   (or 0 if no MMAP support)
*/
...
#if HAVE_MORECORE
#define CALL_MORECORE(S, M) MORECORE(S, M)
#else /* HAVE_MORECORE */
#define CALL_MORECORE(S, M) MFAIL
#endif /* HAVE_MORECORE */
 ...
 mem-> +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  ...
  The exceptions to all this are
     ...
     1. The special chunk `top' is the top-most available chunk (i.e.,
        ...
        system if it is very large (see DL_M_TRIM_THRESHOLD).  In effect,
...
typedef void *(*morecore_pfn_t)(intptr_t increment, void *data);
...
static int change_mparam(int param_number, int value) {
  ...
  init_mparams();
  switch (param_number) {
    case DL_M_TRIM_THRESHOLD:
      mparams.trim_threshold = val;
      return 1;
    case DL_M_GRANULARITY:
      if (val >= mparams.page_size && ((val & (val - 1)) == 0)) {
        mparams.granularity = val;
        return 1;
      } else
        return 0;
    case DL_M_MMAP_THRESHOLD:
...
static void do_check_any_chunk(mstate m, mchunkptr p) {
  dlassert((is_aligned(chunk2mem(p))) || (p->head == FENCEPOST_HEAD));
  dlassert(ok_address(m, p));
}
...
static void do_check_top_chunk(mstate m, mchunkptr p) {
  ...
  dlassert((is_aligned(chunk2mem(p))) || (p->head == FENCEPOST_HEAD));
  dlassert(ok_address(m, p));
  dlassert(sz == m->topsize);
  dlassert(sz > 0);
  dlassert(sz == ((sp->base + sp->size) - (char *)p) - TOP_FOOT_SIZE);
  dlassert(pinuse(p));
  dlassert(!next_pinuse(p));
}
...
static void do_check_mmapped_chunk(mstate m, mchunkptr p) {
  ...
  dlassert(is_mmapped(p));
  dlassert(use_mmap(m));
  dlassert((is_aligned(chunk2mem(p))) || (p->head == FENCEPOST_HEAD));
  dlassert(ok_address(m, p));
  dlassert(!is_small(sz));
  dlassert((len & (mparams.page_size - SIZE_T_ONE)) == 0);
  dlassert(chunk_plus_offset(p, sz)->head == FENCEPOST_HEAD);
  dlassert(chunk_plus_offset(p, sz + SIZE_T_SIZE)->head == 0);
}
...
static void do_check_inuse_chunk(mstate m, mchunkptr p) {
  ...
  dlassert(is_mmapped(p) || pinuse(p) || next_chunk(prev_chunk(p)) == p);
...
static void do_check_free_chunk(mstate m, mchunkptr p) {
  ...
  if (p != m->dv && p != m->top) {
    if (sz >= MIN_CHUNK_SIZE) {
      dlassert((sz & CHUNK_ALIGN_MASK) == 0);
      dlassert(is_aligned(chunk2mem(p)));
      dlassert(next->prev_foot == sz);
      dlassert(pinuse(p));
      dlassert(next == m->top || cinuse(next));
      dlassert(p->fd->bk == p);
      dlassert(p->bk->fd == p);
    } else /* markers are always of size SIZE_T_SIZE */
      dlassert(sz == SIZE_T_SIZE);
  }
}
...
static void do_check_malloced_chunk(mstate m, void *mem, size_t s) {
  if (mem != 0) {
    ...
    dlassert(is_mmapped(p) || sz < (s + MIN_CHUNK_SIZE));
  }
}
...
static void do_check_tree(mstate m, tchunkptr t) {
  ...
  dlassert(tindex == idx);
  dlassert(tsize >= MIN_LARGE_SIZE);
  dlassert(tsize >= minsize_for_tree_index(idx));
  dlassert((idx == NTREEBINS - 1) || (tsize < minsize_for_tree_index((idx + 1))));

  do { /* traverse through chain of same-sized nodes */
    do_check_any_chunk(m, ((mchunkptr)u));
    dlassert(u->index == tindex);
    dlassert(chunksize(u) == tsize);
    dlassert(!cinuse(u));
    dlassert(!next_pinuse(u));
    dlassert(u->fd->bk == u);
    dlassert(u->bk->fd == u);
    if (u->parent == 0) {
      dlassert(u->child[0] == 0);
      dlassert(u->child[1] == 0);
    } else {
      dlassert(head == 0); /* only one node on chain has parent */
      head = u;
      dlassert(u->parent != u);
      dlassert(u->parent->child[0] == u || u->parent->child[1] == u || *((tbinptr *)(u->parent)) == u);
      if (u->child[0] != 0) {
        dlassert(u->child[0]->parent == u);
        dlassert(u->child[0] != u);
        do_check_tree(m, u->child[0]);
      }
      if (u->child[1] != 0) {
        dlassert(u->child[1]->parent == u);
        dlassert(u->child[1] != u);
        do_check_tree(m, u->child[1]);
      }
      if (u->child[0] != 0 && u->child[1] != 0) {
        dlassert(chunksize(u->child[0]) < chunksize(u->child[1]));
      }
    }
    u = u->fd;
  } while (u != t);
...
static void do_check_smallbin(mstate m, bindex_t i) {
  ...
  if (!empty) {
    for (; p != b; p = p->bk) {
      ...
      dlassert(small_index(size) == i);
      dlassert(p->bk == b || chunksize(p->bk) == chunksize(p));
  ...
  if (is_initialized(m)) {
    ...
    while (s != 0) {
      ...
      while (segment_holds(s, q) && q != m->top && q->head != FENCEPOST_HEAD) {
        sum += chunksize(q);
        if (cinuse(q)) {
          dlassert(!bin_find(m, q));
          do_check_inuse_chunk(m, q);
        } else {
          dlassert(q == m->dv || bin_find(m, q));
          dlassert(lastq == 0 || cinuse(lastq)); /* Not 2 consecutive free */
...
static void do_check_malloc_state(mstate m) {
  ...
  if (m->dvsize != 0) { /* check dv chunk */
    ...
    dlassert(m->dvsize == chunksize(m->dv));
    dlassert(m->dvsize >= MIN_CHUNK_SIZE);
    dlassert(bin_find(m, m->dv) == 0);
  }

  if (m->top != 0) { /* check top chunk */
    do_check_top_chunk(m, m->top);
    dlassert(m->topsize == chunksize(m->top));
    dlassert(m->topsize > 0);
    dlassert(bin_find(m, m->top) == 0);
  }

  total = traverse_and_check(m);
  dlassert(total <= m->footprint);
  dlassert(m->footprint <= m->max_footprint);
}
#endif /* DEBUG */
...
#define unlink_small_chunk(M, P, S)                                                                                   \
  {                                                                                                                   \
    ...
    dlassert(P != B);                                                                                                 \
    dlassert(P != F);                                                                                                 \
    dlassert(chunksize(P) == small_index2size(I));                                                                    \
    if (F == B)                                                                                                       \
      clear_smallmap(M, I);                                                                                           \
...
static void *mmap_alloc(mstate m, size_t nb) {
  ...
  if (mmsize > nb) { /* Check for wrap around 0 */
    ...
    if (mm != CMFAIL) {
      ...
      if (mm < m->least_addr) m->least_addr = mm;
...
static void *prepend_alloc(mstate m, char *newbase, char *oldbase, size_t nb) {
  ...
  dlassert((char *)oldfirst > (char *)q);
  dlassert(pinuse(oldfirst));
  dlassert(qsize >= MIN_CHUNK_SIZE);
  ...
  */
  ...
  if (MORECORE_CONTIGUOUS && !use_noncontiguous(m)) {
    ...
    if (ss == 0) { /* First time through or recovery */
      char *base = (char *)CALL_MORECORE(0, m);
      if (base != CMFAIL) {
        asize = granularity_align(nb + TOP_FOOT_SIZE + SIZE_T_ONE);
        /* Adjust to end on a page boundary */
        if (!is_page_aligned(base)) asize += (page_align((size_t)base) - (size_t)base);
        /* Can't call MORECORE if size is negative when treated as signed */
        if (asize < HALF_MAX_SIZE_T && (br = (char *)(CALL_MORECORE(asize, m))) == base) {
          tbase = base;
          tsize = asize;
        }
      }
      ...
      if (asize < HALF_MAX_SIZE_T && (br = (char *)(CALL_MORECORE(asize, m))) == ss->base + ss->size) {
        tbase = br;
        tsize = asize;
      }
    }
      ...
      if (br != CMFAIL) {  /* Try to use/extend the space we did get */
        if (asize < HALF_MAX_SIZE_T && asize < nb + TOP_FOOT_SIZE + SIZE_T_ONE) {
          ...
          if (esize < HALF_MAX_SIZE_T) {
            char *end = (char *)CALL_MORECORE(esize, m);
            if (end != CMFAIL)
              asize += esize;
            else { /* Can't use; try to release */
              (void)CALL_MORECORE(-asize, m);
              br = CMFAIL;
            }
          }
        }
  ...
  if (HAVE_MORECORE && tbase == CMFAIL) { /* Try noncontiguous MORECORE */
    ...
    if (asize < HALF_MAX_SIZE_T) {
      ...
      char *end = CMFAIL;
      ACQUIRE_MORECORE_LOCK();
      br = (char *)(CALL_MORECORE(asize, m));
      end = (char *)(CALL_MORECORE(0, m));
      RELEASE_MORECORE_LOCK();
  ...
  if (tbase != CMFAIL) {
    ...
    if (!is_initialized(m)) { /* first-time initialization */
      ...
      msegmentptr sp = &m->seg;
      while (sp != 0 && tbase != sp->base + sp->size) sp = sp->next;
      if (sp != 0 &&
          //! is_extern_segment(sp) &&
          (sp->sflags & IS_MMAPPED_BIT) == mmap_flag && segment_holds(sp, m->top)) { /* append */
        sp->size += tsize;
        init_top(m, m->top, m->topsize + tsize);
      } else {
        if (tbase < m->least_addr) m->least_addr = tbase;
        sp = &m->seg;
        while (sp != 0 && sp->base != tbase + tsize) sp = sp->next;
  ...
  while (sp != 0) {
    ...
    if (is_mmapped_segment(sp) && !is_extern_segment(sp)) {
      ...
      if (!cinuse(p) && (char *)p + psize >= base + size - TOP_FOOT_SIZE) {
        tchunkptr tp = (tchunkptr)p;
        dlassert(segment_holds(sp, (char *)sp));
        if (p == m->dv) {
          m->dv = 0;
          m->dvsize = 0;
        } else {
...
static int sys_trim(mstate m, size_t pad) {
  ...
  if (pad < MAX_REQUEST && is_initialized(m)) {
      ...
      if (!is_extern_segment(sp)) {
        if (is_mmapped_segment(sp)) {
          ...
          ACQUIRE_MORECORE_LOCK();
          {
            ...
            char *old_br = (char *)(CALL_MORECORE(0, m));
            if (old_br == sp->base + sp->size) {
              char *rel_br = (char *)(CALL_MORECORE(-extra, m));
              char *new_br = (char *)(CALL_MORECORE(0, m));
    ...
    if (released == 0) m->trim_check = MAX_SIZE_T;
  }
...
static void *internal_memalign(mstate m, size_t alignment, size_t bytes) {
  ...
  if (bytes >= MAX_REQUEST - alignment) {
    ...
    if (mem != 0) {
      ...
      dlassert(chunksize(p) >= nb);
      dlassert((((size_t)(chunk2mem(p))) % alignment) == 0);
...
static void **ialloc(mstate m, size_t n_elements, size_t *sizes, int opts, void *chunks[]) {
  ...
  */
  ...
  if (marray != chunks) {
    ...
    if (element_size != 0) {
      dlassert(remainder_size == element_size);
    } else {
      dlassert(remainder_size == request2size(sizes[i]));
    }
  ...
  if (!PREACTION(gm)) {
    ...
    if (bytes <= MAX_SMALL_REQUEST) {
      ...
      if ((smallbits & 0x3U) != 0) { /* Remainderless fit to a smallbin. */
        ...
        p = b->fd;
        dlassert(chunksize(p) == small_index2size(idx));
...
void *dlrealloc(void *oldmem, size_t bytes) {
  if (oldmem == 0) return dlmalloc(bytes);
...
void *dlmemalign(size_t alignment, size_t bytes) { return internal_memalign(gm, alignment, bytes); }
...
size_t dlmalloc_usable_size(void *mem) {
  if (mem != 0) {
    mchunkptr p = mem2chunk(mem);
    if (cinuse(p)) return chunksize(p) - overhead_for(p);
  }
  return 0;
}

int dlmallopt(int param_number, int value) { return change_mparam(param_number, value); }
...
void mspace_set_morecore(mspace msp, morecore_pfn_t amorecore_pfn, void *amorecore_ptr) {
  mstate ms = (mstate)msp;
  ms->morecore_pfn = amorecore_pfn;
  ms->morecore_ptr = amorecore_ptr;
}

void *mspace_get_morecore_ptr(mspace msp) {
  mstate ms = (mstate)msp;
  return ms->morecore_ptr;
}

void mspace_set_morecore_ptr(mspace msp, void *p) {
  mstate ms = (mstate)msp;
  ms->morecore_ptr = p;
}

void mspace_set_morecore_pfn(mspace msp, morecore_pfn_t amorecore_pfn) {
  mstate ms = (mstate)msp;
  ms->morecore_pfn = amorecore_pfn;
}

mspace mspace_from_base(void *base) { return (mspace)chunk2mem(base); }
...
size_t destroy_mspace(mspace msp) {
  ...
  if (ok_magic(ms)) {
    ...
    while (sp != 0) {
      ...
      (void)base;
      sp = sp->next;
      if ((flag & IS_MMAPPED_BIT) && !(flag & EXTERN_BIT) && CALL_MUNMAP(base, size) == 0) freed += size;
    }
  } else {
...
int mspace_mallopt(int param_number, int value) { return change_mparam(param_number, value); }
