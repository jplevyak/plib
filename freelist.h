...
template <class C>
class ClassFreeList : public FreeList {
 public:
  C protoObject;

  C *alloc();

  ClassFreeList(int acount = 64, int aalignment = 16) : FreeList(sizeof(C), acount, aalignment) {}
};

inline void FreeList::init(int asize, int acount, int aalignment) {
  size = asize;
  count = acount;
...
inline void FreeList::xpand() {
  void *last = head;
#if _XOPEN_SOURCE == 600
  if (posix_memalign(&head, alignment, (size * count) + sizeof(void *))) head = 0;
#else
...
template <class C>
inline C *ClassFreeList<C>::alloc() {
