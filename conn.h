...
#define APPEND_TO_BUF(_b, _s)              \
  do {                                     \
    memcpy(_b.end, _s "", sizeof(_s) - 1); \
    _b.end += sizeof(_s) - 1;              \
  } while (0)
#define ALLOC_STRING(_s) alloc_str(_s, sizeof(_s) - 1)
#define LINECMP(_const_string) \
  (((int)sizeof(_const_string "")) - 1 > rbuf.line_len() || STRCMP(rbuf.line, _const_string))
#define LINECASECMP(_const_string) \
  (((int)sizeof(_const_string "")) - 1 > rbuf.line_len() || STRCASECMP(rbuf.line, _const_string))
#define LINEPCMP(_p, _const_string) (((int)sizeof(_const_string "")) - 1 > rbuf.cur - (_p) || STRCMP(_p, _const_string))
#define LINEPCASECMP(_p, _const_string) \
  (((int)sizeof(_const_string "")) - 1 > rbuf.cur - (_p) || STRCASECMP(_p, _const_string))
#define LINEPSKIPSPACE(_p) \
  while ((_p) < rbuf.cur && isspace(*(_p))) (_p)++;
...
class Conn : public ThreadPoolJob {
 public:
  int ifd, ofd;
  ConnFactory *factory;
  buffer_t rbuf, wbuf;
  ...
  int append_to_buf(buffer_t &b, cchar *s) {
    b.end = (byte *)scpy((char *)b.end, s);
    assert(b.end <= b.bufend);
    return 0;
  }
  ...
  int append_string(cchar *str) {
    append_to_buf(wbuf, str);
    return 0;
  }
  int append_string(cchar *str, int len) {
    append_to_buf_n(wbuf, (const byte *)str, len);
    return 0;
  }
  int append_string(cchar *str, cchar *end) {
    append_to_buf_n(wbuf, (const byte *)str, end - str);
    return 0;
  }
  int append_bytes(void *b, int len) {
    append_to_buf_n(wbuf, (const byte *)b, len);
    return 0;
  }
...
#define PUT_STRING(_s) put_string(_s, sizeof(_s) - 1)
  ...
  int put_bytes(void *b, int len) {
    int r;
    if ((r = append_bytes(b, len))) return r;
    return put();
  }
  int put();
...
class Server : public ConnFactory {
 public:
  Server(cchar *aname);
};
...
inline void Conn::check_rbuf() {
  ...
  if (l && rbuf.cur != rbuf.buf) memmove(rbuf.buf, rbuf.cur, l);
  rbuf.end = rbuf.buf + l;
  rbuf.cur = rbuf.buf;
}
