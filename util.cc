...
class UtilService : public Service { public:
...
int bind_port(int port, int protocol, bool nolinger) {
  ...
  if (getsockname(fd, (struct sockaddr *)&name, (socklen_t*)&addrlen))
    PERROR("getsockname");
...
int accept_socket(int socket, bool nolinger, bool tcp_nodelay, int set_buf_size) {
  ...
  if (tcp_nodelay) {
    int one = 1;
    if (setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, (const char *)&one, sizeof(one)))
      PERROR("setsockopt");
  }
  if (nolinger) {
    struct linger linger;
    linger.l_onoff = 1;
    linger.l_linger = 0;
    if (setsockopt(fd, SOL_SOCKET, SO_LINGER, (const char*)&linger, sizeof(struct linger)) < 0)
      PERROR("setsockopt");
  }
  return fd;
}
...
char *memmem(char *m, char *e, char *p, int l) {
  if (l == 1)
    return (char*)memchr(m, *p, e-m);
  while (1) {
    m = (char*)memchr(m, *p, e-m);
    if (!m || m+l > e)
      return 0;
    if (!memcmp(m+1, p+1, l-1))
      return m;
    m++;
  }
  return 0;
...
static int
compar_freer(const void *a, const void *b) {
  freer_t *i = *(freer_t**)a;
  freer_t *j = *(freer_t**)b;
  if (i->t > j->t)
    return 1;
...
void UtilService::start() {
  freer_thread = create_thread(freer_main, 0);
}

void UtilService::stop() {
  pthread_cancel(freer_thread);
}
